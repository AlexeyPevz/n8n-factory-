...
- **Cron Schedule** - каждые 30 мин **ТОЛЬКО в рабочее время (9:00-18:00 пн-пт)**
- **MTProto Message Trigger** - получает ответы от клиентов 
...
- Каждые 30 минут **в рабочее время** берет новые лиды
- Проверяет время: **9:00-18:00 по Москве, пн-пт**
- Определяет лучший канал связи (Telegram → WhatsApp → Email)
- Генерирует персонализированное КП
- Отправляет с задержкой 2 сек между сообщениями (защита от спама)
